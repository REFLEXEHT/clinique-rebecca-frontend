...
export default function Navbar({ onRdvClick, variant = 'public' }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (!(e.target as HTMLElement).closest('.mobile-nav-content')) setMobileOpen(false)
    }
    ...
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-black/40 md:hidden" onClick={() => setMobileOpen(false)}>
          <div className="mobile-nav-content absolute top-[70px] left-0 right-0 bg-white shadow-2xl
