...
export default function AdminUtilisateurs() {
  ...
  const onSave = async (u: any) => {
    ...
    try {
      ...
      const props: any[] = res.data?.propagations || []
      const totalChanged = props.reduce((acc: number, p: any) => acc + (p.result?.changed || 0), 0)
      if (totalChanged > 0) {
  ...
  const onDelete = async (u: any) => {
    ...
    try {
      ...
      toast.error('Impossible de supprimer (protégé ou admin)')
    }
  }
  ...
  return (
    <div style={{ padding: 28 }}>
                ...
                return (
                  <div key={i} style={{ marginBottom: 6 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#16a34a' }}>
                      {typeLabel}
                      {(p.result?.changed || 0) > 0
                        ? ` — ${p.result.changed} enregistrement${p.result.changed > 1 ? 's' : ''} mis à jour`
                        : ' — aucun impact'}
                    </div>
                    {p.result?.detail &&
                      Object.entries(p.result.detail)
                    ...
                    {p.result?.nouvelles_regles_partage && (
                      <div style={{ background: '#eff6ff', borderRadius: 8, padding: '8px 12px', marginTop: 6, fontSize: 12 }}>
                        ...
                        {Object.entries(p.result.nouvelles_regles_partage).map(([acte, pct]) => (
                          <div key={acte} style={{ color: '#1641C8' }}>
                            · {acte} : {pct as string}
                          </div>
                        ))}
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        {[
          { k: 'tous', l: 'Tous' },
          ...Object.entries(ROLES_LABELS).map(([k, v]) => ({ k, l: v.label })),
        ].map((f) => (
          <button
            key={f.k}
        ...
        <button
          onClick={load}
          style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: '#64748b', display: 'flex', alignItems: 'center', gap: 4 }}
        >
          ...
          <thead>
            <tr style={{ background: '#f8fafc' }}>
              {['Nom', 'Email', 'Rôle', 'Spécialité / Type', 'Statut', 'Actions'].map((h) => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', color: '#64748b', fontWeight: 700, fontSize: 12, borderBottom: '1px solid #e2e8f0' }}>
                  {h}
                </th>
              ))}
            </tr>
                  ...
                  {editId === u.id ? (
                      ...
                      {u.role === 'medecin' && (
                        <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 12, color: '#92400e', display: 'flex', gap: 8 }}>
                          ...
                          <div>
                            ...
                            mettra à jour les rendez-vous, profils comptables, tarifs et fiche spécialiste liés.
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 12 }}>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setEditData((d: any) => ({ ...d, telephone: e.target.value }))}
                            style={inp}
                          />
                        </div>
                        {u.role === 'medecin' && (
                          <>
                            <div>
                              ...
                              <select
                                ...
                                onChange={(e) => setEditData((d: any) => ({ ...d, specialite: e.target.value }))}
                                style={inp}
                              >
                            ...
                            <div>
                              <label style={lbl}>Type médecin</label>
                              <select
                                value={editData.type_medecin}
                                onChange={(e) => setEditData((d: any) => ({ ...d, type_medecin: e.target.value }))}
                                style={inp}
                              >
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        <button
                          ...
                          style={{
                            ...
                            background: '#1641C8', color: 'white', border: 'none',
                            borderRadius: 9, padding: '9px 18px', fontWeight: 700,
                            fontSize: 13, cursor: 'pointer', opacity: loading ? 0.7 : 1,
                          }}
                        >
                          <Save size={14} />
                          {u.role === 'medecin' ? 'Enregistrer et propager' : 'Enregistrer'}
                        </button>
                        <button
                    ...
                    <>
                      ...
                      <td style={{ padding: '12px 16px', color: '#64748b', fontSize: 12 }}>
                        ...
                        {u.type_medecin && (
                          <div style={{ color: '#0d9488', fontSize: 11 }}>
                            {TYPES_MEDECIN.find((t) => t.value === u.type_medecin)?.label || u.type_medecin}
                          </div>
                        )}
                      </td>
                      <td style={{ padding: '12px 16px' }}>
                        {u.is_active ? (
                          <span style={{ background: '#f0fdf4', color: '#16a34a', borderRadius: 8, padding: '4px 10px', fontSize: 11, fontWeight: 700 }}>
                            Actif
                          </span>
                        ) : (
                          <button
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button
                            onClick={() => startEdit(u)}
                            style={{ background: '#eff6ff', border: 'none', borderRadius: 8, padding: '6px 9px', cursor: 'pointer', color: '#1641C8' }}
                          >
                            <Edit2 size={13} />
                          </button>
                          {u.role !== 'admin' && (
                            <button
                              onClick={() => onDelete(u)}
                              style={{ background: '#fef2f2', border: 'none', borderRadius: 8, padding: '6px 9px', cursor: 'pointer', color: '#dc2626' }}
                            >
                              <Trash2 size={13} />
                            </button>
                          )}
