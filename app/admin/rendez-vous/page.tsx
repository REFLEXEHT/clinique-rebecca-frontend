'use client'
// app/admin/rendez-vous/page.tsx
        ...
        <div className="flex gap-2">
          {['', 'en_attente', 'confirme', 'annule', 'termine'].map(s => (
            <button key={s} onClick={() => setFilter(s)}
              ...
              {s === '' ? 'Tous' : STATUTS[s as StatutRDV]?.label || s}
            </button>
          ))}
        </div>
      </div>
        ...
        {loading ? (
          ...
          <table className="tbl w-full">
            <thead><tr>
              ...
              <th>Spécialité</th><th>Type</th><th>Paiement</th><th>Statut</th><th>Action</th>
            </tr></thead>
            <tbody>
              {displayRdvs.map(rdv => {
                ...
                return (
                  <tr key={rdv.id}>
                    ...
                    <td>
                      <a href={`https://wa.me/${rdv.patient_telephone.replace(/\D/g,'')}`}
                    ...
                    <td>
                      <select
                        ...
                        className={`text-[12px] font-bold px-2 py-1 rounded-lg border-none
                    ...
                    <td>
                      <button onClick={() => cancel(rdv.id)}
                        className="text-[11.5px] text-red-400 hover:text-red-600
                          font-semibold transition-colors bg-transparent border-none cursor-pointer">
                        Annuler
                      </button>
                    </td>
                  </tr>
