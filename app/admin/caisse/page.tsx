    ...
    <div className="p-7">
      ...
      <div className="grid grid-cols-2 gap-5 mb-6">
        <div className="card p-5"><h4 className="font-bold text-[13.5px] mb-4 flex items-center gap-2"><i className="fa-solid fa-chart-bar text-[#1641C8] text-sm"/>Recettes journalières — 7 jours</h4>
          <div className="h-52">{recChart.length>0?<Bar data={{labels:recChart.map(d=>d.date),datasets:[{data:recChart.map(d=>d.total),backgroundColor:'rgba(34,197,94,0.75)',borderRadius:6}]}} options={{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{ticks:{callback:(v:any)=>`${v/1000}k`,font:{size:11}},grid:{color:'rgba(0,0,0,0.04)'}},x:{grid:{display:false},ticks:{font:{size:11}}}}}} as any/>:<div className="h-full flex items-center justify-center text-slate-300 text-sm">Connectez l'API</div>}</div>
        </div>
