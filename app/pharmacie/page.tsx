'use client'
// app/pharmacie/page.tsx — Espace pharmacien (consultation stocks, pas de modification)
  ...
  useEffect(() => {
    stocksApi.list().then(r=>setStocks(r.data)).catch(()=>{
      ...
      setStocks([
        ...
        { id:5, nom:'Seringues 10ml', categorie:'Matériel', quantite:380, seuil_min:200, prix_unitaire:8, unite:'unité' },
        { id:6, nom:'Masques chirurgicaux', categorie:'Protection', quantite:8, seuil_min:50, prix_unitaire:5, unite:'unité' },
      ])
    })
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="bg-[#0f172a] h-[70px] flex items-center px-6 gap-4">
        ...
        <h1 className="text-white font-bold ml-2">Espace Pharmacie</h1>
      ...
      <div className="p-7">
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-[13px] mt-0.5">
            Consultation des stocks — La modification des quantités est réservée à l'administrateur
          </p>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3.5 mb-5 flex items-center gap-2.5">
          ...
          <span className="text-[#1641C8] font-semibold text-sm">
            En tant que pharmacien, vous pouvez consulter les stocks. Pour modifier les quantités, contactez l'administrateur.
          </span>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-5">
          ...
          <div className="kpi-card"><div className="text-2xl font-black text-green-600 mb-1">{stocks.filter(s=>s.quantite>=s.seuil_min).length}</div><div className="text-xs text-slate-500 font-semibold">En stock</div></div>
          <div className="kpi-card"><div className="text-2xl font-black text-yellow-600 mb-1">{stocks.filter(s=>s.quantite>0&&s.quantite<s.seuil_min).length}</div><div className="text-xs text-slate-500 font-semibold">Stock faible</div></div>
          <div className="kpi-card"><div className="text-2xl font-black text-red-500 mb-1">{stocks.filter(s=>s.quantite===0).length}</div><div className="text-xs text-slate-500 font-semibold">Rupture de stock</div></div>
        </div>

        {/* Filtres */}
        <div className="flex gap-3 mb-4">
          <input value={search} onChange={e=>setSearch(e.target.value)}
            placeholder="Rechercher un médicament..." className="input w-64"/>
          ...
          <table className="tbl w-full">
            ...
            <tbody>
              {filtered.map(s=>{
                ...
                return (
                  <tr key={s.id}>
                    <td className="font-semibold text-[13px]">{s.nom}</td>
                    <td><span className="badge badge-gray">{s.categorie}</span></td>
                    <td>
                      <span className={`font-mono font-bold text-[13px] ${s.quantite===0?'text-red-500':s.quantite<s.seuil_min?'text-yellow-600':'text-green-600'}`}>
                        {s.quantite}
                      </span>
                    </td>
                    <td className="text-[12.5px] text-slate-500">{s.unite}</td>
                    <td className="font-mono text-[12.5px]">{s.prix_unitaire} HTG</td>
                    <td><span className={`badge ${st.cls}`}>{st.label}</span></td>
                  </tr>
                )
              })}
              {filtered.length===0&&<tr><td colSpan={6} className="text-center text-slate-400 py-8 text-sm">Aucun produit trouvé</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
