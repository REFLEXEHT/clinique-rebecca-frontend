...
export default function CaissierPage() {
 ...
 const [showChangePwd, setShowChangePwd] = useState(false)
  ...
  useEffect(() => {
    ...
    const load = () => {
      if (!active) return
      api.get('/caissier/prochain-numero')
        .then(r => {
          const num = r.data?.prochain_numero
          if (num && num.startsWith('#RB-') && active) {
            setPreviewNumero(num)
          } else if (active) {
            setTimeout(load, 2000)
          }
        })
        .catch(() => { if (active) setTimeout(load, 2000) })
    }
    load()
    return () => { active = false }
  }, [])
...
{onglet==='paiement' && (
 ...
 <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'1px solid #f1f5f9',fontSize:12,alignItems:'center'}}>
 <div style={{flex:1}}>
