...
export default function SpecialistePage() {
  ...
  // FIX: Initialiser à null — ne pas lire params.id hors du useEffect
  // Sur Vercel (SSR/hydration), params peut être vide au premier rendu,
  // ce qui donnait Number(undefined) = NaN → SPECS_DATA[NaN] = undefined → "Spécialiste introuvable"
  const [spec, setSpec] = useState<(typeof SPECS_DATA)[number] | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // Lire params.id uniquement dans useEffect (côté client, après hydration)
    ...
    // Pré-remplir immédiatement avec les données statiques pour éviter le flash "introuvable"
    const staticData = SPECS_DATA[id] || null
    setSpec(staticData)

    // Puis enrichir depuis l'API backend (échec silencieux si indisponible)
    specialistesApi.getById(id)
      .then(r => {
        setSpec(() => ({
          ...(SPECS_DATA[id] || {}),
          ...r.data,
          tags: SPECS_DATA[id]?.tags || [],
          bio: SPECS_DATA[id]?.bio || r.data?.bio || '',
        } as (typeof SPECS_DATA)[number]))
      })
      .catch(() => {
        // API indisponible : on garde les données statiques déjà affichées
      })
      .finally(() => setLoading(false))
  }, [params])

  // Spinner de chargement pendant la résolution des params côté client
  if (loading) return (
    <>
      <Navbar onRdvClick={() => setRdvOpen(true)} />
      <div className="min-h-screen flex items-center justify-center pt-[70px]">
        <div className="flex flex-col items-center gap-3 text-slate-400">
          <div className="w-10 h-10 border-4 border-blue-200 border-t-[#1641C8] rounded-full animate-spin" />
