          ...
          <div className="hero-photo-wrap">
            <img
              ...
              onError={(e) => {
                ...
                t.parentElement!.style.background = 'linear-gradient(160deg, #0f1e3d 0%, #1641C8 55%, #0d9488 100%)'
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
            {SERVICES_ACCUEIL.map(s => (
              <Link key={s.titre} href={s.href} style={{ textDecoration: 'none' }}>
                <div
                  className="card"
                  style={{ padding: '28px 24px', transition: 'all 0.22s', cursor: 'pointer', borderColor: '#e2e8f0' }}
                  onMouseEnter={e => {
                    const d = e.currentTarget
                    d.style.transform = 'translateY(-4px)'
                    d.style.boxShadow = '0 12px 40px rgba(22,65,200,0.1)'
                    d.style.borderColor = s.couleur + '40'
                  }}
                  onMouseLeave={e => {
                    const d = e.currentTarget
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {SPECIALITES_VEDETTES.map(s => (
              <Link key={s} href="/specialites" style={{ textDecoration: 'none' }}>
                <div
                  style={{ background: '#f8fafc', borderRadius: 14, padding: '14px 18px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 10, transition: 'all 0.18s' }}
            ...
            {TEMOIGNAGES.map((t, i) => (
              <div key={i} className="card" style={{ padding: '32px 28px' }}>
                <div style={{ display: 'flex', gap: 3, marginBottom: 20 }}>
                  {[...Array(5)].map((_, j) => (
                    <i key={j} className="fa-solid fa-star" style={{ color: '#f59e0b', fontSize: 14 }} />
                  ))}
                </div>
                <p style={{ color: '#334155', fontSize: 15, lineHeight: 1.75, marginBottom: 24, fontStyle: 'italic' }}>
                  "{t.texte}"
                </p>
